...
-- SUM(G) TOTAL_GAMES,
 --SUM(SO) STRIKE_OUTS,
 ROUND(CAST(SUM(SO) AS NUMERIC)/2/CAST(SUM(G)AS NUMERIC)/2,2) AVG_STRIKEOUT_PER_GAME,
-- SUM(HR) HRS,
 ROUND(CAST(SUM(HR) AS NUMERIC)/2/CAST(SUM(G)AS NUMERIC)/2,2) AVG_HR_PER_GAME
FROM 
 TEAMS 
WHERE YEARID >1920
GROUP BY
